...
-- Additive only. Requires existing helpers: is_super_admin(), has_role(text).
...
create policy "ss_write_admin" on public.school_settings
  for all using (
    is_super_admin() or exists (
      select 1 from public.user_roles ur
      where ur.user_id = auth.uid() and ur.school_id = school_settings.school_id and ur.role = 'school_admin'
    )
  );
...
create policy "ay_write_admin" on public.academic_years
  for all using (
    is_super_admin() or exists (
      select 1 from public.user_roles ur
      where ur.user_id = auth.uid() and ur.school_id = academic_years.school_id and ur.role = 'school_admin'
    )
  );
...
create policy "terms_write_admin" on public.terms
  for all using (
    is_super_admin() or exists (
      select 1 from public.user_roles ur
      where ur.user_id = auth.uid() and ur.school_id = terms.school_id and ur.role = 'school_admin'
    )
  );
...
create policy "slots_write_admin" on public.time_slots
  for all using (
    is_super_admin() or exists (
      select 1 from public.user_roles ur
      where ur.user_id = auth.uid() and ur.school_id = time_slots.school_id and ur.role = 'school_admin'
    )
  );
...
create policy "ce_write_admin" on public.calendar_exceptions
  for all using (
    is_super_admin() or exists (
      select 1 from public.user_roles ur
      where ur.user_id = auth.uid() and ur.school_id = calendar_exceptions.school_id and ur.role = 'school_admin'
    )
  );
...
create policy "aj_select_scope" on public.attendance_justifications
  for select using (is_super_admin() or has_role('school_admin'));
...
create policy "ann_write_admin" on public.announcements
  for all using (
    is_super_admin()
    or exists (select 1 from public.user_roles ur where ur.user_id = auth.uid() and ur.school_id = announcements.school_id and ur.role = 'school_admin')
  );
...
create policy "gj_write_admin" on public.generation_jobs
  for all using (
    is_super_admin()
    or exists (select 1 from public.user_roles ur where ur.user_id = auth.uid() and ur.school_id = generation_jobs.school_id and ur.role = 'school_admin')
  );
...
create policy "tv_write_admin" on public.timetable_versions
  for all using (
    is_super_admin()
    or exists (select 1 from public.user_roles ur where ur.user_id = auth.uid() and ur.school_id = timetable_versions.school_id and ur.role = 'school_admin')
  );
